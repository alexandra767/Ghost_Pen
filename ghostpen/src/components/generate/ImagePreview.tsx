        ...
        <div className="px-4 pb-4">
          ...
          {editingPrompt ? (
            <textarea
              ...
              className="w-full bg-background border border-border rounded-lg px-3 py-2 text-xs text-foreground/70 focus:outline-none focus:border-gold/50 min-h-[60px]"
            />
          ) : (
            <p className="text-xs text-foreground/50 leading-relaxed">{imagePrompt}</p>
          )}

          <button
            onClick={() => onRegenerate(editedPrompt)}
            disabled={isGenerating}
            className="flex items-center gap-1.5 mt-3 px-3 py-1.5 rounded-lg text-xs bg-gold/10 text-gold hover:bg-gold/20 transition-colors disabled:opacity-40"
          >
